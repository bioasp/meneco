...
def test_meneco():
    ...
    assert reconstructable_sol == result['Reconstructable targets']
    assert essential_reactions_target_sol == result['Essential reactions']
    assert one_min_sol == result['One minimal completion']
    assert union_sol == result['Union of cardinality minimal completions']
    assert intersection_sol == result['Intersection of cardinality minimal completions']
